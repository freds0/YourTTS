...
# BN phonemizer has compatibility issues with Python 3.9 due to bangla library
try:
...
_base_phonemizers = [ESpeak, Gruut, KO_KR_Phonemizer]
if BN_Phonemizer is not None:
    _base_phonemizers.append(BN_Phonemizer)
PHONEMIZERS = {b.name(): b for b in _base_phonemizers}
...
def get_phonemizer_by_name(name: str, **kwargs) -> BasePhonemizer:
    ...
    if name == "bn_phonemizer":
        if BN_Phonemizer is None:
            raise ValueError(" ❗ Bangla phonemizer is not available. It requires Python 3.10+ or a compatible 'bangla' library.")
